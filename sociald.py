...
def main(_argv):
    ...
    while True:
        ...
        for (id1,ax1,ay1,ax2,ay2),(id2,bx1,by1,bx2,by2) in combinations(objectboxes,2):     # Checks the euclidean distance between any two people
            ...
            if(np.sqrt((x1-x2)**2 +(y1-y2)**2)<75):             # Change this parameter to change the social distancing threshold
                if (id1,ax1,ay1,ax2,ay2) not in risked_people:
                    ...
                    total_riskpeople=total_riskpeople+1
        ...
        for (person_id,x1,y1,x2,y2) in risked_people:
            ...
            cv2.putText(frame,"Person "+str(person_id),(x1,y1-10),0,0.5,(255,255,255),2)
        ...
        if(len(risked_people_id)==0):
            ...
            cv2.rectangle(frame,(0,0),(350,90),colorred,-1)
            cv2.putText(frame,"Social Distancing Violation",(20,30),0,0.75,(255,255,255),2)
            cv2.putText(frame,"Person ID: "+listToStr,(20,50),0,0.5,(255,255,255),2)
            cv2.putText(frame,"Total Violation: "+str(total_riskpeople),(20,70),0,0.5,(255,255,255),2)
            total_riskpeople=total_riskpeople+1
        # calculate frames per second of running detections
        fps = 1.0 / (time.time() - start_time)
        totalfps = totalfps+fps
        print("FPS: %.2f" % fps)
        avgfps = totalfps/frame_num
